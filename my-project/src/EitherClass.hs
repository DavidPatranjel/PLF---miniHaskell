...
fromLeft def valeither = either id (const def) valeither
...
fromRight def valeither = either (const def) id valeither
...
eitherLeftMap f val = either (\x -> left (f x)) (right) val 
...
eitherRightMap f val = either (left) (\x -> right (f x)) val 
...
instance EitherClass CEither where
  left a = CEither (\l r -> l a)
  right b = CEither (\l r -> r b)
